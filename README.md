...
PWA для интервального повторения SAT-лексики (цель 1550+). Замена Anki. Две даты: **PRIMARY_DATE = 03.10.2026** (реальный якорь — регистрация до 18.09) и **EXAM_DATE = 07.11.2026** (зачитываемая суперскор-попытка, на ней завязаны `DUE_CAP`/предэкзаменационный ramp).
...
- **Планировщик** — [ts-fsrs](https://github.com/open-spaced-repetition/ts-fsrs) (FSRS-6), desired retention 0.80 (с 09.09.2026; последние две недели перед попыткой 0.90, `FINAL_RETENTION`). Никакого самописного алгоритма.

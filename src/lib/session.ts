...
export function hasSeparator(list: StudyItem[], i: number, ctx: OrderCtx): boolean {
  /*
   * A12: дрилл - не знакомство, и A6 к нему не применяется. Единица с открытым планом дриллов
   * (`StudyItem.drill`, drill.ts) попадает сюда только одним путём: слово провалило отработку в
   * этом же уроке, и `baseFormat` рисует ему окно «Подзабылось» вместо очередного шага ладдера.
   * Вреда, ради которого стоит A6, здесь нет: слово УЖЕ введено и уже получило настоящую оценку,
   * его карточка и журнал заполнены, и окно без отработки следом - это ещё один показ значения,
   * а не сгоревшее знакомство (тем же словом это сказано в checkA6, session-sim.test.ts).
   * Цена запрета, наоборот, реальная: у слова остаётся долг в DRILL_REPS - 1 отработок, и A6
   * отказывает ему в экране ровно тогда, когда остаток урока короткий, - то есть именно там,
   * где долг уже не вернуть (репро: rand#78 и rand#397 в session-sim, урок обрывался с дриллом
   * в очереди). Потолки окон дрилла не касаются по построению: `overIntroLimit` спрашивает
   * `isFreshNew`, а единица плана дриллов ни разу не New и всегда `introduced`.
   */
  if (list[i].drill !== undefined) return true
...
/**
 * Послабления, которыми вызывающий открывает следующую ступень лестницы добора (B4).
 * По умолчанию урок берёт строгий проход и батч знакомств; пол разрыва - только явно.
 */
export interface PickOpts {
  /** аварийный пол разрыва A2 (30 c, после знакомства 10 c): вся лестница добора пройдена */
  floor?: boolean
  /** батч знакомств A4-bis; выключается, пока у урока есть непройденные ступени лестницы */
  batch?: boolean
}

/**
 * Следующий экран урока. Порядок очереди сохраняется — берётся ПЕРВАЯ допустимая единица.
 * Три прохода, каждый включается только если предыдущий пуст и разрешён опциями:
 *   1) строгий - A2 (60 c) + A3 + A4 + A6, идёт всегда;
 *   2) батч знакомств (A4-bis) - знакомство сразу после знакомства, когда разбавлять нечем.
 *      Отключается `batch: false`: по B4 батч стоит ТРЕТЬЕЙ ступенью лестницы, после готовой
 *      единицы пула и недоработанных сегодняшних слов, и включать его вместе со строгим
 *      проходом значит вводить новое слово там, где урок обязан отработать уже введённое;
 *   3) аварийный пол A2 - включается ТОЛЬКО с `floor`, то есть когда вызывающий уже прошёл
 *      всю лестницу добора (сегодняшние недоработанные, батч знакомств, ранние повторы,
 *      лишнее новое слово) и альтернатив нет вообще. Ожидания на экране нет: простой в
 *      учебном приложении хуже, чем показ на тридцатой секунде вместо шестидесятой.
 * A3 и A6 не смягчаются никогда.
 */
export function pickNext(list: StudyItem[], ctx: OrderCtx, opts: PickOpts = {}): NextPick {
  const passes = opts.batch === false ? [false] : [false, true]
  for (const relaxA4 of passes) {

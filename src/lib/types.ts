...
export interface Settings {
  ...
  typing: boolean // ввод слова по буквам как формат словаря; по умолчанию выключен (см. pickFormat)
}
...
export const DEFAULT_SETTINGS: Settings = {
  ...
  /* Ввод по буквам выключен.
     Он был основным форматом словаря — 271 показ из 472 — и не проверяется на
     SAT нигде: там словарь всегда выбор из четырёх. Отдельно: с 26.07 сломана
     левая рука. Остаётся как явная настройка и для числового ответа (math),
     где однозначность даёт сама задача. */
  typing: false
}

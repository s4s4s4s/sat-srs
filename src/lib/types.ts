...
export const DEFAULT_SETTINGS: Settings = {
  ...
  /* Целевая точность FSRS. 0.9 стояла с первого дня и к 09.09.2026 оказалась
     топтанием на месте: интервалы 2, 11, 46 дней после трёх Good, урок из одних
     и тех же слов. 0.8 даёт 8, 76, 525 дней (замер ts-fsrs, Good подряд, без
     fuzz), то есть выученное слово уходит из ротации, а потолок сроков
     `dueCap` всё равно возвращает его перед попыткой. Последние две недели
     перед попыткой действует `FINAL_RETENTION` (scheduler.ts). Поля в
     интерфейсе нет, до установленного PWA значение доезжает миграцией v9. */

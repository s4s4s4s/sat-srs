...
export async function applyPull(
  ...
  for (const f of fetched) {
    ...
    /* E-синк: ветка create/create годится только для ЗДОРОВОЙ локальной записи - той, которую
       push действительно может взять (`dirty && !broken` в sync.ts). Битую (broken=1) переезд на
       свободный -N путь хоронит навсегда: отправить её нельзя, файла с таким именем в репозитории
       нет и не появится, а значит и починить её некому - тьютор такого файла не видит. Итог -
       вечный `unsyncedCount > 0` и вечный статус `warning` при том, что ученику нечего сделать.
       Поэтому битая запись без sha идёт общей merge-веткой ниже: содержимое берётся у удалённого
       файла, локальный fsrs переживает слияние (mergeCard), карантин снимается сам, как только
       remote цел, а если и remote битый - запись остаётся застрявшей под РЕАЛЬНЫМ путём, который
       тьютор найдёт в vault (stuckMessage). Терять при этом нечего: содержимое битой записи -
       неразобранный текст, которого нет ни в одном файле репозитория. */
    if (cur?.dirty && cur.sha === null && !cur.broken) {
      ...
      // fsrs останется в записи и дождётся починки файла (F29). Сюда же приходит битая локальная
      // запись без sha (E-синк, условие ветки выше): для неё это единственный путь к отправке.

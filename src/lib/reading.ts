...
/**
 * Нижняя и верхняя граница отрывка. Взяты из пассажа настоящего Reading & Writing
 * (25-150 слов, контракт WS10): верхняя граница совпадает с потолком экзаменационного
 * пассажа, нижняя выбрана так, чтобы единица работы не выродилась в пару предложений.
 * Тексты колоды на 22.08.2026 держат 614-1155 слов, заход на весь текст занимал 6-9
 * минут при темпе 95-160 сл/мин, и ноль прочитанных текстов за неделю: единица работы
 * была слишком крупной, чтобы влезть в свободную минуту.
 */
...
/**
 * Текст -> отрывки по 120-150 слов, без разреза предложения (граница та же, что режет
 * контекст карточки, `splitSentences`) и без разреза абзаца сверх необходимого: абзацные
 * переносы внутри отрывка сохраняются (`\n\n`), чтобы `paragraphs()` на экране разобрал
 * отрывок так же, как раньше разбирал весь текст.
 *
 * Копится счётчик слов по предложениям; отрывок закрывается, как только накопление
 * достигло нижней границы, независимо от абзаца, которому принадлежит предложение.
 * Абзацная граница поэтому не обязана совпадать с границей отрывка: связность внутри
 * абзаца создаёт автор текста, а не разбиение на единицы работы. Верхняя граница держит
 * типичный отрывок, но не гарантирована: одно длинное предложение (формула, перечень)
 * может довести отрывок выше 150, потому что резать его пополам нельзя, а закрыть отрывок
 * НЕДОБОРОМ до нижней границы ради формальной верхней хуже, чем немного превысить её.
 *
 * Последний отрывок может выйти короче нижней границы: резать текст ради круглого
 * числа слов, когда резать больше нечего, смысла нет.
 */
export function chunkPassages(text: string): string[] {
  ...
  const closeParagraph = () => {
    if (currentSentences.length) {
      currentParas.push(currentSentences.join(' '))
      currentSentences = []
    }
  }
  const closeChunk = () => {
  ...
  for (const p of paragraphs(text)) {
    for (const sentence of splitSentences(p)) {
      ...
      // Закрываем ровно по достижении нижней границы, не раньше. Предложение не режется,
      // а значит на большом предложении есть только два исхода: не дотянуть до нижней
      // границы или немного перейти верхнюю. Недобор хуже перебора: отрывок короче 120
      // слов уже не единица работы, а обрубок.
      if (words >= CHUNK_MIN_WORDS) closeChunk()
    }
    closeParagraph()
  }
  closeChunk()

  return chunks.map(ps => ps.join('\n\n'))
}

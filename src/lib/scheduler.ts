...
/* Дат две, и путать их дорого.
   ...
   три следствия, каждое проверено на живых данных:
     ...
     — потолок сроков стоял на 31.10, то есть ПОЗЖЕ первой попытки: карточка легально
...
/** Попытки по порядку. Всё, что зависит от «когда экзамен», считается отсюда. */
const ATTEMPTS: readonly Date[] = [PRIMARY_DATE, EXAM_DATE]

/**
 * Ближайшая ещё не наступившая попытка (E3).
 *
 * Дата, к которой готовятся, - не константа, а функция времени: 03.10 до самой
 * первой попытки, 07.11 после неё. Раньше PRIMARY_DATE стояла в планировщике,
 * метрике и на главном экране как единственная точка отсчёта, и 03.10 приложение
 * превращалось в памятник прошедшей попытке - потолок сроков выключался,
 * «до SAT» застывало нулём, готовность считалась на вчера. Правило одно и живёт
 * здесь; копий у него быть не должно.
 *
 * Границу берём по началу дня попытки: 03.10 готовиться уже поздно, в этот день
 * экзамен пишут, и планировать надо на следующую попытку.
 */
...
/* Запас между потолком сроков и попыткой: последний повтор не должен приходиться
   на канун экзамена, если этого можно избежать. */
export const CAP_LEAD_DAYS = 7

/**
 * Потолок интервалов: всё выученное обязано вернуться хотя бы раз до этой даты (E3).
 *
 * Была константа DUE_CAP = 26.09 - неделя до первой попытки. Константа отвечает
 * на вопрос «когда» один раз, а вопрос звучит «сколько осталось до ближайшей
 * попытки», и ответ на него меняется каждый день. Три следствия, каждое видно на
 * живых данных:
 *
 *   - с 27.09 потолок оказывался в прошлом, `clampDueBeforeCap` выключал себя
 *     условием `now >= cap`, и всё со стабильностью выше ~17 дней уезжало за
 *     03.10: самая выученная четверть колоды не освежалась перед попыткой;
 *   - после 03.10 потолка не оставалось вовсе - до 07.11 карточка могла не
 *     вернуться ни разу;
 *   - в последнюю неделю запас в 7 дней держать не из чего: потолок обязан
 *     съехать на канун попытки, иначе повторов до неё просто не будет.
 *
 * Отсюда три положения потолка на одну попытку: неделя запаса, пока она есть;
 * канун попытки в последнюю неделю; потолок СЛЕДУЮЩЕЙ попытки, когда до текущей
 * меньше суток и втиснуть повтор уже некуда. После последней попытки список дат
 * кончается, и потолок остаётся в прошлом - зажимать нечего, пока в ATTEMPTS не
 * появится третья дата.
 */
export function dueCap(now: Date = new Date()): Date {
  for (const attempt of ATTEMPTS) {
    const lead = new Date(attempt.getTime() - CAP_LEAD_DAYS * 86400_000)
    if (now.getTime() < lead.getTime()) return lead
    const eve = new Date(attempt.getTime() - 86400_000)
    if (now.getTime() < eve.getTime()) return eve
  }
  return new Date(EXAM_DATE.getTime() - CAP_LEAD_DAYS * 86400_000)
}

/**
 * Стоп ввода новых - свой у каждого раздела (A8).
 *
 * Слову нужно порядка 21 дня стабильности, чтобы дозреть к первой попытке:
 * введённое позже 19.09 просто не успевает. Правилу пунктуации столько не нужно -
 * оно не дозревает, а знается: разобранное и отработанное правило работает на
 * экзамене через неделю так же, как через месяц. Один общий стоп на колоду
 * закрывал грамматику и логику вместе со словарём и стоил домена SEC (26% RW,
 * ноль оценок за три месяца) ради запаса, который этим разделам не нужен.
 *
 * Граница включает саму дату: с 00:00 в день стопа бюджет раздела уже нулевой,
 * последний рабочий день ввода - накануне.
 */
export const NEW_STOP_BY_SECTION: Record<Section, Date> = {
  rw: new Date(2026, 8, 19),       // словарь: 21 день стабильности до 03.10
  logic: new Date(2026, 8, 26),    // разборы: приём рассуждения, дозревание короче
  grammar: new Date(2026, 8, 30),  // правила: знаются, а не дозревают
  /* математика: приём решения, как правило грамматики, а не слово - знается, а не дозревает.
     До 24.09.2026 стоп стоял 19.09 вместе со словарём, и все 127 карточек раздела, ни разу
     не введённые, встали стеной «Всё повторено» (жалоба Александра 24.09.2026). */
  math: new Date(2026, 8, 30)
}

/** Стоп ввода словаря. Темп (`pace`) и отчёт считаются по словам - им нужна одна дата. */
export const NEW_STOP_DATE = NEW_STOP_BY_SECTION.rw

/** true, пока новые карточки этого раздела ещё разрешено вводить (A8). */
export function newIntroAllowed(now: Date, section: Section): boolean {
  return now.getTime() < NEW_STOP_BY_SECTION[section].getTime()
}

/**
 * Где мы относительно ближайшей попытки. Даты границ - те же, что у потолка сроков
 * и стопов ввода; отдельного календаря у фазы нет.
 *
 *   intake  - ввод новых открыт хотя бы одному разделу;
 *   taper   - ввод закрыт всем, идёт дозревание с недельным запасом;
 *   final   - последняя неделя перед попыткой, потолок сроков стоит на кануне;
 *   between - попыток впереди больше нет.
 *
 * Порядок проверок важен: последняя неделя сильнее открытого ввода грамматики -
 * 27.09 грамматика ещё вводится (A8), но режим дня уже финальный.
 */
export type Phase = 'intake' | 'taper' | 'final' | 'between'

export function phase(now: Date = new Date()): Phase {
  const attempt = nextAttempt(now)
  if (now.getTime() >= attempt.getTime()) return 'between'
  if (now.getTime() >= attempt.getTime() - CAP_LEAD_DAYS * 86400_000) return 'final'
  return SECTIONS.some(s => newIntroAllowed(now, s)) ? 'intake' : 'taper'
}
 ...
 * Easy 1), ввод 8 новых в день до NEW_STOP_DATE, срок зажат потолком dueCap, — даёт к
 ...
 * 135, пик 208 на 25.09 (в этот день упирается в потолок dueCap всё, что
 ...
 * После стопа своего раздела добора нет: этот путь (bonusNew в Review.tsx) - второй вход
 * нового слова в урок, помимо основного бюджета buildQueue, и обязан закрываться тем же
 * правилом, иначе «стоп ввода» держится наполовину. Стоп спрашивается у РАЗДЕЛА каждой
 * карточки (A8), а не один раз на весь вызов: набор бывает смешанным.
 */
export function nextNewItems(cards: CardView[], exclude: Set<string>, limit = 1, now: Date = new Date(), marked: ReadonlySet<string> = new Set()): StudyItem[] {
  return freshItems(expandItems(cards, now), marked)
    .filter(i => i.skill === 'recall' && !exclude.has(itemKey(i)) && newIntroAllowed(now, sectionOf(i.view)))
...
export function buildQueue(
  ...
  const overdueSorted = items
    ...
    .sort((a, b) => a.fsrs.due.getTime() - b.fsrs.due.getTime())
  /* MAX_LEECH_PER_LESSON: потолок стоит на ПЕРВОМ показе после карантина
     (leechReturned === 'first'), а не на любом вернувшемся. `leechReturned`
     держит 'later' всю жизнь флага после истечения карантина, а потолок
     задуман про однообразие ОДНОГО захода - несколько первых показов подряд
     дают один и тот же формат (reveal/mc), но карточка, уже прошедшая свой
     первый показ и вернувшаяся в обычный оборот, такой опасности не несёт и
     не обязана застревать сверх пятёрки, пока долг в homeCounts продолжает
     считаться. 'first' сверх потолка в этот заход не берётся вовсе: карточку
     не трогают (ни fsrs, ни leech-флаг), она остаётся overdue как есть и
     попадёт в урок следующим вызовом buildQueue, как только в голове среза
     найдётся место. Отсекать её приходится ДО среза по
     MAX_REVIEW_PER_LESSON/reviewsLeftToday, а не полагаться на то, что общий
     потолок урока обрежет сам: колода может быть меньше урочного потолка (29
     повторов при лимите 60), и тогда без явного отсечения все девять первых
     показов пиявок попали бы в один урок разом. 'later' конкурирует в общем
     reviewCap на равных со всей остальной просрочкой. */
  let вернувшихсяВзято = 0
  const overdue: StudyItem[] = []
  for (const i of overdueSorted) {
    if (leechReturned(i.view, now) === 'first') {
  ...
  /* Переполнение (WS9): overdue длиннее того, что РЕАЛЬНО пойдёт в срез (reviewCap), и притом
     ...
     нейтральным. Сравнивать нужно с reviewCap, а не с dayLeft: dayLeft - остаток на весь
     день (до 180), а срез в этом заходе режется по reviewCap (не больше
     MAX_REVIEW_PER_LESSON минус то, что уже забрали провалы) - первый урок финальной недели
     с dayLeft=180 и reviewCap=60 при 100 просроченных иначе получал overflow=false и старый
     срез по due без учёта корпусного веса, хотя 40 карточек всё равно отсекались. Вне
     финального окна остаток вернётся следующим уроком (см. комментарий выше про
     MAX_LEECH_PER_LESSON), и менять порядок незачем - прежнее поведение (по due) сохраняется
     буквально. */
  const overflow = overdue.length > reviewCap && phase(now) === 'final'
  ...
  // Стоп ввода (A8): после стопа СВОЕГО раздела карточка в очередь не попадает, что бы ни

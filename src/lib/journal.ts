...
/* Основы колоды по стеммеру `lightStem` (`stem.ts`), кэшированные по самому объекту `deck` -
   без кэша `deckHasWord` пересчитывала бы стем каждого слова колоды на КАЖДЫЙ вызов (линейный
   обход, вызывается по разу на каждую уникальную помеченную лемму в `markDigest`). Ключ - сам
   `ReadonlySet`: колода пересобирается заново при каждой синхронизации/загрузке (новый объект),
   поэтому смена колоды сама инвалидирует запись, а WeakMap не держит старые деки в памяти. */
const deckStemCache = new WeakMap<ReadonlySet<string>, Set<string>>()
function deckStems(deck: ReadonlySet<string>): Set<string> {
  let stems = deckStemCache.get(deck)
  if (!stems) {
    stems = new Set<string>()
    for (const d of deck) stems.add(lightStem(d))
    deckStemCache.set(deck, stems)
  }
  return stems
}
 ...
 * тексте (мн. число, притяжательное 's, причастие), точным совпадением с карточкой не
 * сойдётся никогда: `assumptions` и `assumption` - разные строки. Второй заход сравнивает
 * основу (`lightStem`, `stem.ts`) отметки с основой каждого слова колоды - без него такие
 * формы уезжали в «кандидаты (в колоде нет)», хотя карточка уже есть (F41 в контракте, 24%
 * списка добавления на живой колоде до правки). Стеммер намеренно консервативный - без
 * деривационных суффиксов Портера (-ive/-ion): именно они склеивали `relative` с `relate`,
 * пряча настоящий кандидат на новую карточку под видом уже введённого слова (J2).
 */
export function deckHasWord(deck: ReadonlySet<string>, ...forms: (string | undefined)[]): boolean {
  ...
  const stems = deckStems(deck)
  return candidates.some(c => stems.has(lightStem(c)))
}
...
/**
 * Темп чтения текста — слов в минуту, 0 если мерить не из чего.
 *
 * Берётся ПОСЛЕДНЕЕ прочтение со временем: текст можно перечитать, и осмысленно то, как
 * он читается сейчас, а не как читался в первый раз. Строки до 22.08.2026 поля `read_s`
 * не имеют вовсе (D3: схема расширяется добавлением полей) — такой текст честно отдаёт 0,
 * и отчёт ставит прочерк вместо выдуманного числа.
 *
 * Зачем это здесь: ступени текстов двигать больше не по чему. «Отмечено мало слов» говорит
 * лишь о лексике; текст можно взять, разбирая его по словарю полчаса, и порог понятности
 * этого не увидит. Экзамен ограничен временем — темп и есть та величина, ради которой
 * чтение вообще меряется.
 */
export function readingPaceWpm(lines: JournalLine[], slug: string, words: number): number {
  if (words <= 0) return 0
  let sec = 0
  for (const l of lines) if (l.type === 'reading' && l.slug === slug && l.read_s) sec = l.read_s
  return sec > 0 ? Math.round(words / (sec / 60)) : 0
}

...
function sectionStopChecks(): void {
  ...
  // 24.09.2026: математика перестала дозревать как слово - её стоп теперь как у грамматики
  // (30.09, знается - не дозревает), а не вместе со словарём (19.09)
  assert(newIntroAllowed(день(29, 9), 'math') && !newIntroAllowed(день(30, 9), 'math'),
    'A8: математика закрывается вместе с грамматикой (30.09), а не со словарём')
  ...
  assert(!в2009.has('rw'), `20.09: словарь уже закрыт, получено ${[...в2009].join(',')}`)
  assert(в2009.has('grammar') && в2009.has('logic') && в2009.has('math'),
    `20.09: грамматика, логика и математика (стоп 30.09, как у грамматики) обязаны вводиться, получено ${[...в2009].join(',')}`)

  const в2709 = разделыНовых(день(27, 9))
  assert([...в2709].sort().join(',') === 'grammar,math',
    `27.09: логика уже закрыта (26.09), открыты грамматика и математика (обе до 30.09), получено ${[...в2709].join(',')}`)

  assert(разделыНовых(день(1, 10)).size === 0, '01.10: ввод закрыт всем разделам (грамматика и математика закрылись 30.09)')
  ...
  assert(homeCounts(колода, 8, день(20, 9)).newAvail === 5,
    `homeCounts 20.09: доступны 5 новых (грамматика 2, логика 2, математика 1 - её стоп теперь 30.09), получено ${homeCounts(колода, 8, день(20, 9)).newAvail}`)
  assert(homeCounts(колода, 8, день(27, 9)).newAvail === 3,
    `homeCounts 27.09: логика закрыта, остаются грамматика (2) и математика (1), получено ${homeCounts(колода, 8, день(27, 9)).newAvail}`)
  ...
  console.log('  ✓ A8: стоп ввода свой у раздела (слова 19.09, логика 26.09, грамматика и математика 30.09), очередь и главная согласны')
  passed++

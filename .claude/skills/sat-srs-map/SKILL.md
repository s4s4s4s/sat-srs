...
`npm test` = `test/session-sim.test.ts` (симуляция сессий, инварианты очереди и форматов) + `test/metrics.test.ts`. Прогон стоит первым шагом в `deploy.yml`: красный тест блокирует выкатку. Проверки помечены кодами (C1, C3, C8…) — код в тесте и в комментарии к коду в `src/` указывают друг на друга.
